...
class PasswordResetTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create(
            email="harel@harelmalka.com", name="Joe Public"  # /PS-IGNORE
        )
    ...
    def test_reset_request(self):
        ...
        assert bool(
            reset.token
        )  # Checking that the method actually generates and attaches a token to the object
    ...
    def test_reset_is_valid(self):
        ...
        self.assertTrue(
            PasswordResetRequest.objects.validate_token(
                token=token, user_pk=self.user.pk, validate_only=True
            )
        )
    ...
    def test_reset_is_invalid_because_of_wrong_token(self):
        ...
        self.assertFalse(
            PasswordResetRequest.objects.validate_token(
                token="123das", user_pk=self.user.pk, validate_only=True
            )
        )
