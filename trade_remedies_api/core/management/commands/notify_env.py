    ...
    def handle(self, *args, **options):
        ...
        for template in templates.get("templates", []):
            ...
            if template_name.startswith("TR_"):
                ...
                print(f"    {template_name} -> {sys_param_name}")
                try:
                    sysparam = SystemParameter.objects.get(key=sys_param_name)
                    if sysparam.value != template_id:
                        print(f"        Changing from {sysparam.value} to {template_id}")
                        sysparam.value = template_id
                        sysparam.save()
                except SystemParameter.DoesNotExist:
                    print(f"    {sys_param_name} does not already exist.")
            else:
                print(f"Template {template_name} deviates from TR_ naming convention")
