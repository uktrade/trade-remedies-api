...
class Command(BaseCommand):
    help = (
        "Command to fix the incorrect organisation data that was getting confused with "
        "registration data"
    )
        ...
        with open(csv_file) as csv_file:
            ...
            for row in csv_reader:
                ...
                for org in organisation_objects:
                    org.name = new_name
                    self.stdout.write(f"Organisation {current_name} updated to {new_name}")
                    org.save()
                ...
                for s_org in submission_objects:
                    s_org.name = new_name
                    self.stdout.write(f"Submission org_name {current_name} updated to {new_name}")
                    s_org.save()
