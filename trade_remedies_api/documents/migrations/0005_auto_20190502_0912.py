# Generated by Django 2.0.13 on 2019-05-02 09:12
...
def migrate_case_documents(apps, schema_editor):
    try:
        ...
        case_index = {}
        print("Indexing old case documents")
        ...
        print(f"Indexed {len(case_index)} cases")
        for _, params in case_index.items():
            ...
            bundle.save()
            print(f"Bundle created for {bundle.case} (created ?= {created})")
            bundle.documents.clear()
            print(f"Adding {len(params['documents'])} documents to bundle")
            for doc in params["documents"]:
                bundle.documents.add(doc)
    except Exception as exc:
        print(f"Failed. Are we in test? If so that's ok... (reason: {exc})")
